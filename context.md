...
- **Project standing**: Baseline, CAE, and SimCLR pipelines are present; CAE appears to be the strongest completed path so far. MoCo v2 is partially implemented, but currently stands at the "code written, experiment not completed" stage.
- **MoCo status**: `models/moco_model.py` builds successfully in the project `venv`, the core pretraining path has been corrected, and the rebuilt `dataset/Pretrain_Manifest.csv` now excludes SICAP test images. A full MoCo experiment has still not been completed, and no downstream MoCo fine-tune/eval pipeline exists yet.
- **Protocol status**: The train/validation/test split has been corrected in code. Training now uses `TrainSplit.csv`, checkpoint selection uses `Val.csv`, and `Test.csv` is reserved for final evaluation.
- **MoCo implementation gaps**: The main remaining gap is incomplete downstream integration: there is still no MoCo fine-tune/eval pipeline comparable to CAE and SimCLR, so MoCo cannot yet be included in a fair end-to-end comparison.
- **Analysis/reporting gaps**: `evaluation/moco/tsne_moco.py` can mislabel SICAP test patches, and `evaluation/shared/compare_models.py` currently relies on partial hard-coded metrics instead of fully reading real outputs from all methods.
- **Immediate next step**: Finish the MoCo downstream pipeline, then regenerate comparisons using the corrected train/validation/test protocol and only actual saved outputs.
